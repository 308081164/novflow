...
export default function OutlinePage() {
  ...
  const loadPlansAndBook = useCallback(async () => {
    const [b, planList] = await Promise.all([api.book(id), api.chapterPlans(id)]);
    setBook(b);
    setPlans(planList);
    return { book: b, plans: planList };
  }, [id]);
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      // 先加载书籍与规划；同步设定为尽力而为，失败不得清空大纲页
      await loadPlansAndBook();
      try {
        const sync = await api.syncBookSettings(id);
        setSyncInfo(sync);
        if (sync.cards_applied > 0) {
          await loadPlansAndBook();
        }
      } catch {
        /* sync optional */
  ...
  const withOutline = useMemo(() => plans.filter(planHasOutlineContent), [plans]);
  const timelineChapters = withOutline.map(planToTimelineChapter);

  const plannedTotal = book?.planned_chapters || book?.target_chapters || plans.length || 0;
  const outlinePlanned = book?.outline_planned_count ?? withOutline.length;
  ...
  return (
    <div>
      ...
      {error && (
        <div className="mb-4 rounded-lg border border-rose-200 bg-rose-50 px-4 py-2 text-sm text-rose-800">{error}</div>
      )}
      ...
      {loading ? (
        ...
        <div className="card p-8 text-center text-sm text-slate-500">
          尚无章节大纲（已规划 {outlinePlanned} / 全书 {plannedTotal} 章）。在{" "}
              ...
              {withOutline.map((p) => (
                <tr key={p.chapter_no} className="hover:bg-slate-50">
                  ...
                  <td className="px-4 py-3 text-slate-600">
                    {p.plot_points || p.scene || (p.meta_json?.events as string[] | undefined)?.join("；") || "—"}
                  </td>
